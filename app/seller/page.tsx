...
// Removed unused imports: BarChart3, Settings, HelpCircle
...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    ...
    // If token is missing, redirect to signin immediately
    if (!token) {
      console.warn("Supplier token not found. Redirecting to login...");
      router.push("/signin");
      return;
    }
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (!res.ok) {
          // If API returns 401, it might mean token is invalid/expired
          if (res.status === 401) {
            localStorage.removeItem("supplier_token"); // Clear invalid token
            router.push("/signin");
        ...
        // REMOVED: dashboardReadyRef.current = true; 
        // We no longer need this flag; the socket listener handles state updates safely without it.
  ...
  useEffect(() => {
    const sellerStr = localStorage.getItem("supplier");
    if (!sellerStr) return;
    ...
    // DYNAMIC URL: Uses the current domain. 
    // Falls back to localhost:3000 if window.location is unavailable (rare).
    const socketUrl = process.env.NEXT_PUBLIC_SITE_URL || window.location.origin || "http://localhost:3000";

    const socket = io(socketUrl, {
      path: "/socket.io",
      // Removed strict transports: ["websocket"] to allow fallback for better network compatibility
    });
    ...
    socket.on("orderPlaced", (order) => {
      ...
      // This ensures that even if the initial fetch is still pending, 
      // we don't lose the update, or crash if prev is null.
          ...
          notifications: [
            {
              id: Date.now(),
              title: "New Order Received",
              message: `Qty: ${order.quantity || 1}, Size: ${order.size || 'N/A'}`,
              time: "Just now",
              type: "order",
            },
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      <aside className={`${
      ...
      } hidden lg:flex flex-col bg-white border-r border-gray-100 transition-all duration-300 sticky top-0 h-screen z-40 shadow-[4px_0_24px_rgba(0,0,0,0.02)]`}>
        ...
        <nav className="flex-1 px-2 py-6 space-y-6 overflow-y-auto">
                ...
                <div className="pl-12 mt-1 space-y-1">
                  <button
                    onClick={() => {
                      ...
                      handleNavigation('/seller/customer_order')
                    }}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200"
                  >
                  ...
                  <button
                    onClick={() => {
                      ...
                      handleNavigation('/seller/inventory_management')
                    }}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200"
                  >
                    <Package2 size={16} />
                  ...
                  <button
                    onClick={() => {
                      ...
                      handleNavigation('/seller/order_history')
                    }}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200"
                  >
                    <Clock size={16} />
                  ...
                  <button
                    onClick={() => {
                      setActivePage('status-update')
                      handleNavigation('/seller/status_update')
                    }}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200"
                  >
                    <RefreshCcw size={16} />
          ...
          <div className={`space-y-1 ${!isSidebarOpen ? 'flex flex-col items-center' : ''}`}>
            ...
            <button
              onClick={() => handleNavigation('/open-points')}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${
                activePage === 'open-points'
                  ? 'text-amber-600 bg-amber-50/50 font-medium border-l-4 border-amber-500'
                  : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'
              }`}
            >
          ...
          {isSidebarOpen ? (
            <div className="relative">
              <button
                ...
                className="w-full px-3 py-3 bg-white border border-gray-200 hover:border-gray-300 rounded-xl transition-all flex items-center gap-3 group shadow-sm"
              >
                ...
                <div className="flex-1 min-w-0 text-left">
                  <p className="font-semibold text-sm text-gray-900 truncate">{dashboardData.seller.name}</p>
                  <p className="text-[11px] text-gray-500 truncate">{dashboardData.seller.email}</p>
                </div>
              ...
              {isProfileDropdownOpen && (
                ...
                setIsProfileDropdownOpen(false)
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 text-red-600 hover:bg-red-50 transition-colors text-sm border-t border-gray-100"
                  >
                    <LogOut size={18} />
                    <span>Logout</span>
                  </button>
            ...
            <div className="relative">
              <button
                onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)}
                className="w-9 h-9 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center mx-auto hover:scale-105 transition-transform shadow-md shadow-blue-500/30"
              >
                <User size={18} className="text-white" />
              </button>

              {/* Profile Dropdown for Collapsed Sidebar */}
              {isProfileDropdownOpen && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden animate-in slide-in-from-bottom-4 fade-in duration-200">
                  <div className="px-3 py-2 border-b border-gray-100 bg-gray-50">
                    <p className="font-semibold text-xs text-gray-900 truncate">{dashboardData.seller.name}</p>
                    <p className="text-[10px] text-gray-600 truncate">{dashboardData.seller.email}</p>
                  </div>
                  <button
                    onClick={() => {
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50 transition-all">
          <div className="h-full px-8 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 lg:hidden">
              ...
              <div>
                <h1 className="font-bold text-gray-900">AutoRack</h1>
            ...
            <div className="hidden lg:block">
              ...
              <p className="text-sm text-gray-500">Welcome back, <span className="font-medium text-gray-700">{dashboardData.seller.name}</span></p>
            </div>
                ...
                {isNotificationOpen && (
                  <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="p-4 border-b border-gray-50 flex justify-between items-center">
                      ...
                      <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full font-medium">{dashboardData.notifications.length} New</span>
                    </div>
                        ...
                        >
                          <div className="flex justify-between items-start gap-3">
                            <div className="flex-1">
                              <p className="font-medium text-sm text-gray-900 group-hover:text-blue-600 transition-colors">{notification.title}</p>
                    ...
                    <div className="p-3 border-t border-gray-50 bg-gray-50/50 rounded-b-2xl">
                      <button className="w-full text-center text-sm font-medium text-blue-600 hover:text-blue-700 py-2 transition-colors">
            ...
            {isMobileMenuOpen && (
              <div className="absolute top-20 left-0 right-0 bg-white border-b border-gray-100 p-4 lg:hidden shadow-lg z-40">
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    onClick={handleLogout}
                    className="flex items-center gap-3 bg-red-50 hover:bg-red-100 text-red-600 p-3 rounded-lg transition-colors font-medium"
                  >
                    <LogOut size={18} />
                    <span>Logout</span>
                  </button>
        ...
        {error && (
          <div className="mx-8 mt-6 p-4 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-between">
            ...
            <button onClick={() => setError(null)} className="text-blue-400 hover:text-blue-600">
              <X size={18} />
            </button>
          </div>
        )}
              ...
              {stats.map((stat, index) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded-md bg-gray-50 ${stat.color}`}>
                      {stat.change}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                {cards.map((card, index) => (
                  <div
                    ...
                    className={`group bg-white rounded-2xl p-6 border ${card.borderColor} hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer`}
                  >
                    <div className={`w-12 h-12 bg-gradient-to-br ${card.color} rounded-xl flex items-center justify-center mb-4 shadow-lg shadow-blue-500/20 group-hover:scale-110 transition-transform duration-300`}>

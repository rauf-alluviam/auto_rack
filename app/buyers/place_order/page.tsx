...
useEffect(() => {
  ...
  // In PlaceOrderPage component, replace the useEffect that sets up the user
useEffect(() => {
  // Try both token names to be safe
  const token = localStorage.getItem("buyer_token") || localStorage.getItem("token");
  ...
  try {
    ...
    // localStorage.setItem("token", token); // Ensure we have a consistent token name
    
    fetchUserOrders(token, decoded.id);
  } catch (err) {
...
const fetchUserOrders = async (token: string, userId: string) => {
  try {
    console.log("Fetching orders for user:", userId);
    
    const response = await fetch(`/api/orders?buyerId=${userId}`, {
      method: "GET",
      headers: {
...
const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    // ✅ Refresh orders & inventory after placing order
    await fetchUserOrders(token, userId);
    await fetchInventory();

    setIsSubmitted(true);
  } catch (error) {
    console.error("Error placing order:", error);
    alert(`Failed to place order: ${error instanceof Error ? error.message : error}`);
  } finally {
    setLoading(false);
  }
};

...
export default function BuyerDashboard() {
  ...
  const handleOrderUpdated = (updatedFields: Partial<Order> & { _id?: string }) => {
    ...
    // Only update if this order belongs to the current buyer
    if (updatedFields.buyer && updatedFields.buyer !== buyerData?._id) {
      console.log("Ignoring update for another buyer's order");
      return;
    }
    
    // If the update includes an _id, use it to find and update the order efficiently
    if (updatedFields._id) {
      setOrders((prev) =>
        prev.map((order) => 
          order._id === updatedFields._id 
            ? { ...order, ...updatedFields } as Order 
            : order
        )
      );
  ...
  const handleOrderPlaced = (newOrder: Order) => {
    ...
    // Only add if this order belongs to the current buyer
    if (newOrder.buyer && newOrder.buyer !== buyerData?._id) {
      console.log("Ignoring order for another buyer");
      return;
    }
  ...
  useEffect(() => {
    ...
    // Cleanup on unmount or when token/buyer changes
    return () => {
      if (!socket) return
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2.5 w-64 lg:w-80 bg-gray-50/80 border border-gray-200 rounded-xl
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    {filteredOrders.map((order) => (
                      <div
                        key={order._id}
                        className="border border-gray-200 rounded-lg p-4 sm:p-6 hover:shadow-md transition-shadow duration-200 relative"
                      >
                        {/* STATUS BADGE - TOP RIGHT */}
                        <span
                          className={`absolute right-4 top-4 px-3 py-1 rounded-full text-xs font-semibold capitalize
                            ${
                              order.is_accepted === "Pending"
                                ? "bg-yellow-100 text-yellow-700"
                                : order.is_accepted === "Accepted"
                                ? "bg-blue-100 text-blue-700"
                                : order.is_accepted === "In production"
                                ? "bg-purple-100 text-purple-700"
                                : order.is_accepted === "Quality Check"
                                ? "bg-orange-100 text-orange-700"
                                : order.is_accepted === "Packaging"
                                ? "bg-amber-100 text-amber-700"
                                : order.is_accepted === "Shipped"
                                ? "bg-indigo-100 text-indigo-700"
                                : order.is_accepted === "Delivered"
                                ? "bg-green-100 text-green-700"
                                : order.is_accepted === "Rejected"
                        ...
                        <div className="flex flex-col space-y-3">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-4">
                            ...
                            {order.estimated_delivery && (
                              <div className="flex items-center">
                                <Clock className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400 mr-2" />
                                <span className="text-xs sm:text-sm text-gray-600">

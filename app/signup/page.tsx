...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  if (!name.trim()) { setMessage("Please enter your name."); setFieldErrors(prev => ({ ...prev, name: true })); return }
  if (!nameRegex.test(name.trim())) { setMessage("Full Name can only contain alphabets and spaces."); setFieldErrors(prev => ({ ...prev, name: true })); return }
  if (!CompanyName.trim()) { setMessage("Please enter your Company Name."); setFieldErrors(prev => ({ ...prev, companyName: true })); return }
  if (!email.trim()) { setMessage("Please enter your email."); setFieldErrors(prev => ({ ...prev, email: true })); return }
  if (!emailRegex.test(email)) { setMessage("Please enter a valid email address."); setFieldErrors(prev => ({ ...prev, email: true })); return }
  if (!passwordRegex.test(password)) { setMessage("Password must be at least 8 characters long, include uppercase, lowercase, number, and special character."); setFieldErrors(prev => ({ ...prev, password: true })); return }
  if (password !== confirmPassword) { setMessage("Passwords do not match."); setFieldErrors(prev => ({ ...prev, confirmPassword: true })); return }
  ...
  try {
    ...
    console.log("Signup response data:", data) // <-- open DevTools and verify token exists here

    // Use backend userType when present. Fall back to form user's value or 'buyer'
    const roleFromBackend = data?.user?.userType || data?.user?.role
    const role = (roleFromBackend && roleFromBackend.toString().trim()) || userType || "buyer"
    ...
    if (typeof window !== "undefined" && token) {
      ...
      // backwards-compatible role-specific keys if you use them elsewhere
